...
impl UserIdentity {
    ...
    pub(crate) fn identity_matches_certificate(
        &self,
        certificate: String,
    ) -> Result<()> {
    ...
    fn get_attribute_value(
        ...
        attribute.attr_value().as_string()
            .or_else(|_e| attribute.attr_value().as_printablestring().map(|g| g.string()))
            .map_err( |_e|
                SmartIdClientError::FailedToValidateSessionResponseCertificate(
...
mod tests {
    ...
    fn test_check_identity_against_certificate() {
        let user_identity = UserIdentity {
            ...
            identity_code: "PNOBE-98021273111".to_string(),
        };
